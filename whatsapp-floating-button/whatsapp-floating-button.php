...
class WhatsApp_Floating_Button {
    ...
    public static function get_whatsapp_url( $phone, $message, $include_url ) {
        ...
        if ( $include_url ) {
            ...
            if ( ! empty( $current_url ) ) {
                $final_message .= PHP_EOL . PHP_EOL . 'Enviado desde:' . PHP_EOL . $current_url;
            }
        }

        $url .= '?text=' . rawurlencode( $final_message );
    ...
    public static function get_button_html( $args ) {
        ...
        ob_start();
        ?>
                <a href="<?php echo esc_attr( $url ); ?>" class="<?php echo esc_attr( $class ); ?>" target="_blank" rel="nofollow" style="<?php echo esc_attr( $style ); ?>">
